...
func (bq *BookQuery) loadUser(ctx context.Context, query *UserQuery, nodes []*Book, init func(*Book), assign func(*Book, *User)) error {
	...
	query.Where(func(s *sql.Selector) {
		joinT := sql.Table(book.UserTable)
		s.Join(joinT).On(s.C(user.FieldID), joinT.C(book.UserPrimaryKey[1]))
		s.Where(sql.InValues(joinT.C(book.UserPrimaryKey[0]), edgeIDs...))
		columns := s.SelectedColumns()
		s.Select(joinT.C(book.UserPrimaryKey[0]))
