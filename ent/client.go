...
import (
	...
	"github.com/ilcm96/dku-aegis-library/ent/migrate"
...
// BookLogClient is a client for the BookLog schema.
type BookLogClient struct {
	config
}

// NewBookLogClient returns a client for the BookLog from the given config.
func NewBookLogClient(c config) *BookLogClient {
	return &BookLogClient{config: c}
}

// Use adds a list of mutation hooks to the hooks stack.
// A call to `Use(f, g, h)` equals to `booklog.Hooks(f(g(h())))`.
func (c *BookLogClient) Use(hooks ...Hook) {
	c.hooks.BookLog = append(c.hooks.BookLog, hooks...)
}

// Intercept adds a list of query interceptors to the interceptors stack.
// A call to `Intercept(f, g, h)` equals to `booklog.Intercept(f(g(h())))`.
func (c *BookLogClient) Intercept(interceptors ...Interceptor) {
	c.inters.BookLog = append(c.inters.BookLog, interceptors...)
}

// Create returns a builder for creating a BookLog entity.
func (c *BookLogClient) Create() *BookLogCreate {
	mutation := newBookLogMutation(c.config, OpCreate)
	return &BookLogCreate{config: c.config, hooks: c.Hooks(), mutation: mutation}
}

// CreateBulk returns a builder for creating a bulk of BookLog entities.
func (c *BookLogClient) CreateBulk(builders ...*BookLogCreate) *BookLogCreateBulk {
	return &BookLogCreateBulk{config: c.config, builders: builders}
}

// MapCreateBulk creates a bulk creation builder from the given slice. For each item in the slice, the function creates
// a builder and applies setFunc on it.
func (c *BookLogClient) MapCreateBulk(slice any, setFunc func(*BookLogCreate, int)) *BookLogCreateBulk {
	rv := reflect.ValueOf(slice)
	if rv.Kind() != reflect.Slice {
		return &BookLogCreateBulk{err: fmt.Errorf("calling to BookLogClient.MapCreateBulk with wrong type %T, need slice", slice)}
	}
	builders := make([]*BookLogCreate, rv.Len())
	for i := 0; i < rv.Len(); i++ {
		builders[i] = c.Create()
		setFunc(builders[i], i)
	}
...
// Update returns an update builder for BookLog.
func (c *BookLogClient) Update() *BookLogUpdate {
	mutation := newBookLogMutation(c.config, OpUpdate)
	return &BookLogUpdate{config: c.config, hooks: c.Hooks(), mutation: mutation}
}

// UpdateOne returns an update builder for the given entity.
func (c *BookLogClient) UpdateOne(bl *BookLog) *BookLogUpdateOne {
	mutation := newBookLogMutation(c.config, OpUpdateOne, withBookLog(bl))
	return &BookLogUpdateOne{config: c.config, hooks: c.Hooks(), mutation: mutation}
}

// UpdateOneID returns an update builder for the given id.
func (c *BookLogClient) UpdateOneID(id int) *BookLogUpdateOne {
	mutation := newBookLogMutation(c.config, OpUpdateOne, withBookLogID(id))
	return &BookLogUpdateOne{config: c.config, hooks: c.Hooks(), mutation: mutation}
}

// Delete returns a delete builder for BookLog.
func (c *BookLogClient) Delete() *BookLogDelete {
	mutation := newBookLogMutation(c.config, OpDelete)
	return &BookLogDelete{config: c.config, hooks: c.Hooks(), mutation: mutation}
}

// DeleteOne returns a builder for deleting the given entity.
func (c *BookLogClient) DeleteOne(bl *BookLog) *BookLogDeleteOne {
	return c.DeleteOneID(bl.ID)
}

// DeleteOneID returns a builder for deleting the given entity by its id.
func (c *BookLogClient) DeleteOneID(id int) *BookLogDeleteOne {
	builder := c.Delete().Where(booklog.ID(id))
	builder.mutation.id = &id
	builder.mutation.op = OpDeleteOne
	return &BookLogDeleteOne{builder}
}

// Query returns a query builder for BookLog.
func (c *BookLogClient) Query() *BookLogQuery {
	return &BookLogQuery{
		config: c.config,
		ctx:    &QueryContext{Type: TypeBookLog},
		inters: c.Interceptors(),
	}
}

// Get returns a BookLog entity by its id.
func (c *BookLogClient) Get(ctx context.Context, id int) (*BookLog, error) {
	return c.Query().Where(booklog.ID(id)).Only(ctx)
}

// GetX is like Get, but panics if an error occurs.
func (c *BookLogClient) GetX(ctx context.Context, id int) *BookLog {
	obj, err := c.Get(ctx, id)
	if err != nil {
		panic(err)
	}
...
// Hooks returns the client hooks.
func (c *BookLogClient) Hooks() []Hook {
	return c.hooks.BookLog
}

// Interceptors returns the client interceptors.
func (c *BookLogClient) Interceptors() []Interceptor {
	return c.inters.BookLog
}

func (c *BookLogClient) mutate(ctx context.Context, m *BookLogMutation) (Value, error) {
	switch m.Op() {
	case OpCreate:
		return (&BookLogCreate{config: c.config, hooks: c.Hooks(), mutation: m}).Save(ctx)
	case OpUpdate:
		return (&BookLogUpdate{config: c.config, hooks: c.Hooks(), mutation: m}).Save(ctx)
	case OpUpdateOne:
		return (&BookLogUpdateOne{config: c.config, hooks: c.Hooks(), mutation: m}).Save(ctx)
	case OpDelete, OpDeleteOne:
		return (&BookLogDelete{config: c.config, hooks: c.Hooks(), mutation: m}).Exec(ctx)
	default:
		return nil, fmt.Errorf("ent: unknown BookLog mutation op: %q", m.Op())
	}
}
...
// QueryBook queries the book edge of a User.
func (c *UserClient) QueryBook(u *User) *BookQuery {
	query := (&BookClient{config: c.config}).Query()
	query.path = func(context.Context) (fromV *sql.Selector, _ error) {
		id := u.ID
		step := sqlgraph.NewStep(
			...
			sqlgraph.To(book.Table, book.FieldID),
			sqlgraph.Edge(sqlgraph.M2M, true, user.BookTable, user.BookPrimaryKey...),
		)
