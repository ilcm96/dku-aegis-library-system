...
func (uu *UserUpdate) AddBook(b ...*Book) *UserUpdate {
	ids := make([]int, len(b))
	for i := range b {
		ids[i] = b[i].ID
	}
...
func (uu *UserUpdate) RemoveBook(b ...*Book) *UserUpdate {
	ids := make([]int, len(b))
	for i := range b {
		ids[i] = b[i].ID
	}
...
func (uu *UserUpdate) sqlSave(ctx context.Context) (n int, err error) {
	...
	if uu.mutation.BookCleared() {
		edge := &sqlgraph.EdgeSpec{
			Rel:     sqlgraph.M2M,
			Inverse: true,
			Table:   user.BookTable,
			Columns: user.BookPrimaryKey,
			Bidi:    false,
	...
	if nodes := uu.mutation.RemovedBookIDs(); len(nodes) > 0 && !uu.mutation.BookCleared() {
		edge := &sqlgraph.EdgeSpec{
			Rel:     sqlgraph.M2M,
			Inverse: true,
			Table:   user.BookTable,
	...
	if nodes := uu.mutation.BookIDs(); len(nodes) > 0 {
		edge := &sqlgraph.EdgeSpec{
			Rel:     sqlgraph.M2M,
			Inverse: true,
			Table:   user.BookTable,
...
// AddBook adds the "book" edges to the Book entity.
func (uuo *UserUpdateOne) AddBook(b ...*Book) *UserUpdateOne {
	ids := make([]int, len(b))
	for i := range b {
		ids[i] = b[i].ID
	}
...
// RemoveBook removes "book" edges to Book entities.
func (uuo *UserUpdateOne) RemoveBook(b ...*Book) *UserUpdateOne {
	ids := make([]int, len(b))
	for i := range b {
		ids[i] = b[i].ID
	}
...
func (uuo *UserUpdateOne) sqlSave(ctx context.Context) (_node *User, err error) {
	...
	if nodes := uuo.mutation.RemovedBookIDs(); len(nodes) > 0 && !uuo.mutation.BookCleared() {
		edge := &sqlgraph.EdgeSpec{
			Rel:     sqlgraph.M2M,
			Inverse: true,
			Table:   user.BookTable,
	...
	if nodes := uuo.mutation.BookIDs(); len(nodes) > 0 {
		edge := &sqlgraph.EdgeSpec{
			Rel:     sqlgraph.M2M,
			Inverse: true,
			Table:   user.BookTable,
